...
const wrapHttp = require('../src/wrapHttp');

describe('http instrumentation - integration test', () => {
    ...
    tracer.scoped(() => {
      const apiServer = api.listen(0, () => {
        ...
        const req = zipkinHttp.request({
          ...
          expect(annotations[2].annotation.value).to.equal(`http://127.0.0.1:${apiPort}/weather?index=10&count=300`);
  ...
  it('should support on response event', done => {
    tracer.scoped(() => {
      const apiServer = api.listen(0, () => {
        ...
        const zipkinHttp = wrapHttp(http, {tracer, serviceName, remoteServiceName});
        const url = `http://127.0.0.1:${apiPort}/weather?index=10&count=300`;
        zipkinHttp.get(url).on('response', () => {
          const annotations = record.args.map(args => args[0]);
          const initialTraceId = annotations[0].traceId.traceId;
          annotations.forEach(ann => expect(ann.traceId.traceId)
            .to.equal(initialTraceId).and
    ...
    tracer.scoped(() => {
      const apiServer = api.listen(0, () => {
        ...
        const req = zipkinHttp.request({
          ...
          expect(annotations[2].annotation.annotationType).to.equal('BinaryAnnotation');
          expect(annotations[2].annotation.key).to.equal('http.url');
          expect(annotations[2].annotation.value).to.equal(`http://127.0.0.1:${apiPort}/doesNotExist`);
