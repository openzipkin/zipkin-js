...
const ExpressHttpProxyInstrumentation = require('../src/expressHttpProxyInstrumentation');
...
describe('express http proxy instrumentation - integration test', () => {
  it('should add headers to requests', done => {
    ...
    tracer.scoped(() => {
      ...
      const expressHttpProxyInstrumentation = new ExpressHttpProxyInstrumentation(tracer, nameOfApp, nameOfRemoteService);

      const apiServer = api.listen(0, () => {
        const app = express();
        const apiPort = apiServer.address().port;
        app.use(proxy(`127.0.0.1:${apiPort}`, {
            decorateRequest: (proxyReq, originalReq) => expressHttpProxyInstrumentation.decorateAndRecordRequest(proxyReq, originalReq),
            intercept: (rsp, data, originalReq, res, callback) => {
              expressHttpProxyInstrumentation.recordResponse(rsp, originalReq);
              callback(null, data);
            }
          })
        );
        const appServer = app.listen(0, () => {
          ...
          fetch(url).then(res => res.json()).then((res) => {
            ...
            annotations.forEach(ann => expect(ann.traceId.traceId).to.equal(annotations[0].traceId.traceId).and.to.have.lengthOf(16));
